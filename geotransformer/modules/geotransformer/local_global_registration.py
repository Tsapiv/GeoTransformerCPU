...
class LocalGlobalRegistration(nn.Module):
    ...
    def compute_correspondence_matrix(self, score_mat, ref_knn_masks, src_knn_masks):
        ...
        ref_indices = torch.arange(ref_length).view(1, ref_length, 1).expand(batch_size, -1, self.k)  # (B, N, K)
        ...
        src_indices = torch.arange(src_length).view(1, 1, src_length).expand(batch_size, self.k, -1)  # (B, K, N)
    ...
    def convert_to_batch(ref_corr_points, src_corr_points, corr_scores, chunks):
        ...
        indices1 = torch.arange(3).unsqueeze(0).expand(indices.shape[0], 3)  # (3,) -> (total, 3)

        batch_ref_corr_points = torch.zeros(batch_size * max_corr, 3)
        ...
        batch_src_corr_points = torch.zeros(batch_size * max_corr, 3)
        ...
        batch_corr_scores = torch.zeros(batch_size * max_corr)
