...
def extract_correspondences_from_scores(
    ...
    ref_indices = torch.arange(ref_length)
    ...
    if mutual or bilateral:
        ...
        src_indices = torch.arange(src_length)
...
def get_node_overlap_ratios(
    ...
    ref_overlap_masks = torch.zeros(ref_points.shape[0] + 1)  # pad for following indexing
    src_overlap_masks = torch.zeros(src_points.shape[0] + 1)  # pad for following indexing
